...
@section('title', 'Edit Organizer - Admin Panel')
...
<div style="margin-bottom: 2rem; border-bottom: 2px solid var(--corporate-red); padding-bottom: 15px; display: flex; justify-content: space-between; align-items: flex-end;">
    <div>
        <h1 style="color: var(--corporate-red); font-size: 1.8rem; margin: 0; text-transform: uppercase; letter-spacing: 1px;">Edit Organizer</h1>
        <p style="color: #666; margin-top: 5px;">Update an existing organizer's details.</p>
    </div>
    ...
    <form action="{{ route('admin.organizers.update', $organizer->id) }}" method="POST">
        ...
        <div class="responsive-grid" style="margin-bottom: 40px;">
            ...
            <div class="form-group">
                ...
                <input type="text" name="organization" id="organization" required value="{{ old('organization', $organizer->organization) }}" placeholder="e.g. Acme Corporation"
                    style="width: 100%; padding: 14px 20px; border: 2px solid #eee; border-radius: 10px; font-size: 1rem; outline: none; transition: 0.3s; box-sizing: border-box;"
                    onfocus="this.style.borderColor='var(--corporate-red)'" onblur="this.style.borderColor='#eee'">
            ...
            <div class="form-group">
                ...
                <input type="email" name="email" id="email" required value="{{ old('email', $organizer->email) }}" placeholder="organizer@example.com"
                    style="width: 100%; padding: 14px 20px; border: 2px solid #eee; border-radius: 10px; font-size: 1rem; outline: none; transition: 0.3s; box-sizing: border-box;"
                    onfocus="this.style.borderColor='var(--corporate-red)'" onblur="this.style.borderColor='#eee'">
            ...
            <div class="form-group">
                ...
                <div style="display: flex; align-items: stretch; border: 2px solid #eee; border-radius: 10px; overflow: hidden; transition: border-color 0.3s; box-sizing: border-box;" onfocusin="this.style.borderColor='var(--corporate-red)'" onfocusout="this.style.borderColor='#eee'">
                    ...
                    <input type="tel" name="phone_number" id="phone_number" required value="{{ old('phone_number', substr($organizer->phone ?? '', 4)) }}" placeholder="712345678" 
        ...
        <div style="border-top: 1px solid #eee; padding-top: 30px; display: flex; justify-content: flex-end; gap: 15px; flex-wrap: wrap;">
            <a href="{{ route('admin.organizers.index') }}" class="btn" style="background: #f5f5f5; color: #666; border: none; padding: 12px 30px; border-radius: 6px; font-weight: bold; cursor: pointer; text-decoration: none; flex: 1; text-align: center; min-width: 150px;">CANCEL</a>
