...
@section('title', 'Search Results - Organizer Panel')
...
<div style="display: flex; flex-direction: column; gap: 40px;">
    ...
    @php $hasResults = ($events->count() > 0 || $attendees->count() > 0); @endphp
    ...
    @if(!$hasResults)
        <div style="background: white; padding: 80px 30px; border-radius: 12px; border: 1px solid #e2e8f0; text-align: center; color: #64748b; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
            ...
            <a href="{{ route('dashboard') }}" style="display: inline-block; margin-top: 25px; padding: 12px 30px; background: var(--corporate-red); color: white; text-decoration: none; border-radius: 8px; font-weight: 800; font-size: 0.85rem; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">BACK TO DASHBOARD</a>
        </div>
    @endif
    ...
    @if($events->count() > 0)
        <div style="display: flex; flex-direction: column; gap: 25px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                <h3 style="margin: 0; font-size: 1rem; color: #1e293b; font-weight: 800; text-transform: uppercase;">My Matching Events ({{ $events->count() }})</h3>
                ...
                <div class="event-search-card" style="background: white; border-radius: 16px; border: 1px solid #e2e8f0; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.03); display: flex; flex-direction: column;">
                    ...
                    <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 30px; padding: 30px;" class="event-search-grid">
                        ...
                        <div>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 25px;">
                                <div style="background: #f8fafc; padding: 12px; border-radius: 8px; border: 1px solid #f1f5f9;">
                                    <div style="font-size: 0.65rem; color: #94a3b8; font-weight: 800; text-transform: uppercase;">Location</div>
                            ...
                            <h5 style="margin: 0 0 12px; font-size: 0.8rem; color: #1e293b; font-weight: 800; text-transform: uppercase;">Member Register Snapshot</h5>
                                            ...
                                            @foreach($event->registrations->take(5) as $reg)
                                                <tr style="border-bottom: 1px solid #f8fafc;">
                                                    ...
                                                    <td style="padding: 10px 15px; text-align: right;">
                                                        @if($reg->attended)
                                                            <span style="color: #059669; font-weight: 800; font-size: 0.7rem;">PRESENT</span>
                                                        @else
                                                            <span style="color: #94a3b8; font-weight: 800; font-size: 0.7rem;">EXPECTED</span>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endforeach
                        ...
                        <div style="background: #FFF5F5; padding: 25px; border-radius: 12px; border: 1px solid #f9dcdc;">
                            <h5 style="margin: 0 0 10px; font-size: 0.8rem; color: var(--corporate-red); font-weight: 800; text-transform: uppercase;">Promotion Details</h5>
                            <p style="font-size: 0.85rem; color: #64748b; line-height: 1.5; margin-bottom: 20px;">Use the link below to promote your event and collect registrations.</p>
                            ...
                            <div style="display: flex; flex-direction: column; gap: 10px;">
                                <div style="display: flex; gap: 8px;">
                                    ...
                                    <button onclick="copyEventLink('{{ $event->id }}')" id="copy-btn-{{ $event->id }}" style="padding: 8px 12px; background: #1e293b; color: white; border: none; border-radius: 6px; cursor: pointer;">
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: #f8fafc; color: #64748b; border-bottom: 2px solid #f1f5f9;">
                                ...
                                <th style="padding: 15px 25px; text-align: left; font-size: 0.75rem; text-transform: uppercase; font-weight: 800;">Contact Info</th>
                            ...
                            @foreach($attendees as $att)
                                <tr style="border-bottom: 1px solid #f8f9fa;">
                                    <td style="padding: 15px 25px;">
                                        ...
                                        <div style="font-size: 0.7rem; color: var(--corporate-red); font-weight: 800;">ID: {{ $att->access_code }}</div>
                                    </td>
                                    <td style="padding: 15px 25px; font-weight: 600; color: #475569;">{{ $att->organization ?? 'Personal' }}</td>
                                    <td style="padding: 15px 25px;">
                                    ...
                                    <td style="padding: 15px 25px; text-align: right;">
                                        <a href="{{ route('organizer.attendees.edit', $att->id) }}" style="color: var(--corporate-red); font-weight: 800; text-decoration: none; font-size: 0.85rem;">View Profile</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
